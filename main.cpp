...
void guardar(string**,int,vector<char*>& paths);
string** cargar(int&,vector<char*>& paths);
void guardarpaths(vector<char*>&);
void abrirpaths(vector<char*> paths);
...
void guardar(string** tablero, int turno, vector<char*>& paths){
	ofstream escriba;
...
string** cargar(int& turno, vector<char*>& paths){
	int choice;
	...
	ifstream archivo(paths[choice]);
	if(archivo.good()){
		archivo.open(paths[choice]);
		for (int i = 0; i < 8; i++){
			for (int j = 0; j < 8; j++){
				archivo >> tablero[i][j];
			}
...
void guardarpaths(vector<char*>& paths){
	ofstream escriba;
	int size = paths.size();
...
bool jaquemate(string** tablero,int turno){
	...
	bool evt1,evt2,evt3,evt4,evt5,evt6,evt7,evt8;
		...
		y2 = y;
		if(x2 - 1 >= 0 && y2 == y){
			evt1 = confirmarjaque(tablero,turno,x,y);
		}
		x2 = x;
		y2 = y;
		if(x2 + 1 <= 7 && y2 == y){
			evt2 = confirmarjaque(tablero,turno,x,y);
		}
		x2 = x;
		y2 = y;
		if(x2 == x && y2 + 1 <= 7){
			evt3 = confirmarjaque(tablero,turno,x,y);
		}
		x2 = x;
		y2 = y;
		if(x2 == x && y2 - 1 >= 0){
			evt4 = confirmarjaque(tablero,turno,x,y);
		}
		x2 = x;
		y2 = y;
		if(x2 + 1 <= 7 && y2 + 1 <= 7){
			evt5 = confirmarjaque(tablero,turno,x,y);
		}
		x2 = x;
		y2 = y;
		if(x2 - 1 >= 0 && y2 - 1 >= 0){
			evt6 = confirmarjaque(tablero,turno,x,y);
		}
		x2 = x;
		y2 = y;
		if(x2 - 1 >= 0 && y2 + 1 <= 7){
			evt7 = confirmarjaque(tablero,turno,x,y);
		}
		x2 = x;
		y2 = y;
		if(x2 + 1 <= 7 && y2 - 1 >= 0){
			evt8 = confirmarjaque(tablero,turno,x,y);
		}
